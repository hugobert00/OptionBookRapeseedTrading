...
EXPIRY_DATES = {
    2026: {2: 20, 5: 19, 8: 21, 11: 20},
    2027: {2: 19, 5: 18, 8: 20, 11: 19},
    2028: {2: 24, 5: 23, 8: 18, 11: 17},
    2029: {2: 23, 5: 22, 8: 24, 11: 23},
    2030: {2: 22, 5: 21, 8: 23, 11: 22},
    2031: {2: 21, 5: 20, 8: 22, 11: 21},
    2032: {2: 19, 5: 18, 8: 20, 11: 19},
    2033: {2: 18, 5: 19, 8: 20, 11: 18}, 
    2034: {2: 24, 5: 18, 8: 23, 11: 17}
}
...
def single_delta(instrument_type, expiry, F_market, K, v, T, qty, contract_multiplier=1.0):
    ...
    - contract_multiplier : the notional value of the contract which is of 50 on ECO 
    cf : https://live.euronext.com/fr/product/commodities-futures/ECO-DPAR
    """
...
def plot_theta_vs_future_subplots(
    ...
    fig.update_layout(
        height=300 * n,
        title="Theta du portefeuille vs prix du future (EUR / jour, par échéance)",
        showlegend=False,
        template="plotly_white"
    )

    fig.update_xaxes(title_text="Prix du future")
    fig.update_yaxes(title_text="Theta (EUR / jour)")
...
def single_rho(
    ...
    """
    Rhô Black-76 in EUR for + or - 1%  in rates.
    """
    itype = str(instrument_type).strip().lower()
    F = F_market[expiry]
...
def compute_line_rhos(
    ...
    """
    Rhô computation for each line (EUR / 1%).
    """
    def rho_row(row):
        return single_rho(
            instrument_type=row[col_type],
...
def portfolio_rho_by_expiry(
    ...
    """
    Rhô agregation by expiry (EUR / 1%).
    """
    return (
        df
